...
export default function WalletScreen() {
  ...
  const renderTransaction = ({ item }) => (
    <View style={styles.transactionCard}>
      ...
      <Text style={[styles.amountText, { color: item.credit ? Colors.success : Colors.error }]}>
        {item.amount}
      </Text>
    </View>
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.amountRow}>
        {amounts.map((item) => (
          ...
          >
            <Text style={[styles.amountChipText, selectedAmount === item && styles.activeChipText]}>
              ₹{item}
            </Text>
          </TouchableOpacity>
        ))}
...
const styles = StyleSheet.create({
  ...
  header: { paddingTop: Platform.OS === "android" ? 50 : 20, marginBottom: 20 },
  ...
  amountChipText: { fontWeight: "600", color: Colors.text },
